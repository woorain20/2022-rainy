<script>
    ...
    async function add(){
		...
		await fetch(post,{
			...
			body:JSON.stringify({
				done:board.done, date : board.date, content : board.content, title: board.title, workplace: board.workplace, password: board.password
			})
		})
        boards=[board,...boards]
...
<div>
    <h1>게시판 글쓰기</h1>
    <div>
        <input type="text" placeholder="작성자" bind:value="{title}">
