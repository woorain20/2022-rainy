    ...
    public partial class PatientInfo : Form
    {
        public PatientInfo()
        {
            ...
            try
            {
                ...
                textBox_Chart.Text = DataManager.treatments[0].chartNum.ToString();
            }
            catch (Exception)
            {
        ...
        private void dataGridView_PatientList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                textBox_Chart.Text = patients.chartNum.ToString();
            }
            catch (Exception)
            {
        ...
        private void pCode_delete(string chartNum, string command)
        {
            chartNum = chartNum.Trim();

            string contents = "";

            DataManager.TSave(command, chartNum, out contents);
            button_Reset.PerformClick();
            MessageBox.Show(contents);
            writeLog(contents);
        }
        ...
        {
            pCode_delete(textBox_Chart.Text, "delete");
        }
    }
}
