<script>
    ...
    onMount(async()=>{
    const res=await fetch("https://apis.data.go.kr/1613000/BusRouteInfoInqireService/getRouteNoList?serviceKey=x0KzQoj%2FBIV99brth41ohqqH0C5620X1PHjoinCE0HLzbf0J99nvL%2FZo3TCCzAbXXPr%2Bw8iS8bomh8QnO9kqpQ%3D%3D&pageNo=1&numOfRows=482&_type=json&cityCode=26")
    bus=await res.json()
    busInfo=bus.response.body.items.item
    console.log(busInfo)
    })
    onMount(async()=>{
    const res=await fetch("https://apis.data.go.kr/1613000/BusSttnInfoInqireService/getSttnNoList?serviceKey=x0KzQoj%2FBIV99brth41ohqqH0C5620X1PHjoinCE0HLzbf0J99nvL%2FZo3TCCzAbXXPr%2Bw8iS8bomh8QnO9kqpQ%3D%3D&pageNo=1&numOfRows=3756&_type=json&cityCode=26")
